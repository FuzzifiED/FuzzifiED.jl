...
* `nm :: Int64` and `ne :: Int64` specifies the number of orbitals and the number of particles.
* `k :: Int64` and `r :: Int64` specifies the admissibility condition. 
* `lz2 :: Int64` specifies twice the angular momentum in ``z``-direction ``2L^z``.

# Output

* `st :: Matrix{Float64}` is the normalized states in the basis `bs` given by a ``\\text{dim}×N_{\\text{root}}`` matrix where each column specifies a state.
"""
