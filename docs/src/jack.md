...
```
The core function that generates the Jack states is [`GetJackStates`](@ref). The quantum Hall states are specified by a pair of integers $(k,r)$ corresponding to an admissibility condition. We list several common instances below.
...
```
where $\tilde{\mu}$ go through the re-arrangements of $\mu$. _E. g._
...
```
in the representation of occupation number, _i. e._, in a window of $r$ consecutive orbitals, there are maximally $k$ particles. 
...
* B. A. Bernevig, N. Regnault, Anatomy of Abelian and non-Abelian fractional quantum Hall states, [Phys. Rev. Lett. **103**, 206801 (2009)](https://doi.org/10.1103/PhysRevLett.103.206801), [arXiv:0902.4320](https://arxiv.org/abs/0902.4320).
